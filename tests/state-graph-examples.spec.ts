...
describe('Packaged state-graph examples', () => {
  ...
  it.each([true, false])('tests webhook review through its manual entry and resumes the selected branch for approval=%s', async approved => {
    ...
    const resumed = await executeWorkflow(definition, { checkpoint: paused.checkpoint!, resumeValues: { review: approved } }, engine)
    expect(resumed.status).toBe('SUCCESS')
    expect(resumed.output).toBe(approved)
    expect(resumed.nodes.find(node => node.nodeId === 'accepted')?.status).toBe(approved ? 'SUCCESS' : 'SKIPPED')
    expect(resumed.nodes.find(node => node.nodeId === 'declined')?.status).toBe(approved ? 'SKIPPED' : 'SUCCESS')
  })
})
